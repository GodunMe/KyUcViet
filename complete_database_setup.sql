-- =============================================================================
-- SCRIPT THIẾT LẬP DATABASE HOÀN CHỈNH CHO VIETNAMESE MEMORIES
-- =============================================================================
-- Gộp từ: exe201_fixed.sql + checkin_database_setup.sql
-- Tạo ngày: 2025-09-28
-- Mô tả: Script hoàn chỉnh để thiết lập toàn bộ database với chức năng check-in
-- 
-- Chạy script này để:
-- 1. Tạo các bảng chính của hệ thống
-- 2. Tạo các bảng cho chức năng check-in
-- 3. Thêm dữ liệu mẫu cho museum và artifacts
-- 4. Thiết lập quy tắc check-in
-- =============================================================================
...
--
-- Host: localhost:3306
-- Generation Time: Sep 28, 2025 at 02:46 PM
...
-- =============================================================================
-- PHẦN 1: TẠO CÁC BẢNG CHÍNH CỦA HỆ THỐNG (theo thứ tự dependency)
-- =============================================================================

-- -----------------------------------------------------------------------------
-- Bảng users (phải tạo đầu tiên vì là parent table)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Bảng museum (tạo thứ 2 vì artifact và quiz phụ thuộc vào nó)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Bảng option (phụ thuộc question)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Bảng artifact (phụ thuộc museum)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Bảng museum_media (phụ thuộc museum)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Bảng useranswer (phụ thuộc users, quiz, question, option)
-- -----------------------------------------------------------------------------
...
-- =============================================================================
-- PHẦN 2: TẠO CÁC BẢNG CHO CHỨC NĂNG CHECK-IN
-- =============================================================================

-- -----------------------------------------------------------------------------
-- Bảng checkins (bảng chính lưu thông tin check-in)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Bảng checkin_photos (lưu nhiều ảnh cho mỗi check-in)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Bảng museum_checkin_rules (quy tắc check-in cho từng bảo tàng)
-- -----------------------------------------------------------------------------
...
CREATE TABLE `museum_checkin_rules` (
    ...
    `MinTimeBetweenCheckins` INT NOT NULL DEFAULT 1800,
...
-- -----------------------------------------------------------------------------
-- Bảng daily_checkin_limits (giới hạn check-in hàng ngày theo user)
-- -----------------------------------------------------------------------------
...
-- =============================================================================
-- PHẦN 3: THIẾT LẬP INDEXES VÀ CONSTRAINTS
-- =============================================================================
...
-- =============================================================================
-- PHẦN 4: AUTO_INCREMENT SETTINGS
-- =============================================================================
...
-- =============================================================================
-- PHẦN 5: FOREIGN KEY CONSTRAINTS
-- =============================================================================
...
-- Thêm các bảo tàng test cho chức năng check-in
...
-- =============================================================================
-- PHẦN 7: THIẾT LẬP QUY TẮC CHECK-IN
-- =============================================================================
...
SELECT COUNT(*) AS TotalArtifacts FROM artifact;
SELECT COUNT(*) AS CheckinTablesCreated FROM INFORMATION_SCHEMA.TABLES 
WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME LIKE '%checkin%';
SELECT COUNT(*) AS CheckinRulesConfigured FROM museum_checkin_rules;

-- Hiển thị các bảo tàng và quy tắc check-in của chúng
SELECT 
    m.MuseumID,
    m.MuseumName,
    m.Address,
    COALESCE(r.MaxCheckinPerDay, 'Not Set') AS MaxCheckinPerDay,
    COALESCE(r.MinTimeBetweenCheckins, 'Not Set') AS MinTimeBetweenCheckins,
    COALESCE(r.DaysBetweenRevisit, 'Not Set') AS DaysBetweenRevisit
FROM museum m 
...
-- =============================================================================
-- 
-- Database đã được thiết lập hoàn chỉnh với:
-- ✅ Tất cả bảng chính của hệ thống (users, museum, quiz, etc.)
-- ✅ Các bảng cho chức năng check-in (checkins, checkin_photos, etc.)
-- ✅ Dữ liệu mẫu đầy đủ (users, museums, artifacts)
-- ✅ Quy tắc check-in được cấu hình
-- ✅ Foreign key constraints đúng thứ tự
-- ✅ DROP TABLE IF EXISTS để tránh lỗi khi chạy lại
-- 
-- Bạn có thể:
-- 1. Import file này vào phpMyAdmin để tạo database hoàn chỉnh
-- 2. Test tất cả chức năng check-in với dữ liệu mẫu
-- 3. Quản lý bảo tàng qua admin panel
-- 4. Kiểm tra logs check-in trong các bảng tương ứng
-- 
-- =============================================================================